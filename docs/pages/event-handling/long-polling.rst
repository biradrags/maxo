...
Long Polling (длинный опрос) — это самый простой способ получения обновлений от сервера. Бот периодически отправляет запросы к API, спрашивая: "Есть что-то новое?". Если новых событий нет, сервер "держит" соединение открытым некоторое время (timeout), ожидая их появления.

Этот метод идеально подходит для разработки и простых ботов, так как не требует публичного IP-адреса или настройки SSL-сертификатов (в отличие от Webhook).
...
.. note::
    ...
    Для масштабирования (запуска нескольких экземпляров бота) или работы под высокой нагрузкой **необходимо использовать Webhook**. В этом случае сервер Max.ru будет отправлять каждое обновление только один раз на ваш endpoint, и вы сможете эффективно распределять нагрузку между воркерами (например, с помощью Nginx или Kubernetes).
...
Метод `run` принимает несколько полезных аргументов для настройки поллинга:
...
- ``drop_pending_updates`` (bool, по умолчанию False): Если `True`, бот при запуске пропустит все обновления, которые накопились, пока он был выключен. Полезно при разработке, чтобы не обрабатывать кучу старых команд.
