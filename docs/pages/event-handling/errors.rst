...
.. code-block:: python
    ...
    @router.error()
    async def global_error_handler(event: ErrorEvent, facade):
        # Логируем ошибку
...
.. code-block:: python
    ...
    async def access_denied_handler(event: ErrorEvent, facade):
        ...
...
2.  **facade**: Объект :class:`~maxo.utils.facades.updates.error.ErrorEventFacade`. Удобная обертка.
