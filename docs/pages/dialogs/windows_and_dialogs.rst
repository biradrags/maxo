...
``Window`` — это базовый кирпичик интерфейса. Каждое окно соответствует одному сообщению бота с клавиатурой. Окно привязывается к конкретному состоянию конечного автомата (``State``).
...
``Dialog`` — это объединение окон в логический процесс. Он отвечает за маршрутизацию и хранение общего состояния.
